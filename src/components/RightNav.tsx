    ...
    <div className="hidden fixed lg:flex flex-col items-center bg-white max-h-[90vh] min-w-xs right-0 mt-16 border-l border-gray-200 p-4 select-none scrollbar-none overflow-y-auto">
        ...
        <ul className="flex flex-col gap-2 mb-3">
            {
                TrendingTopics.map((topic) => (
                    <li key={topic.id} className="w-full flex items-center justify-between text-gray-500 text-sm px-4 hover:ml-1 hover:text-[#27aa83] transition duration-300 cursor-pointer">
                        <p>#{topic.category}</p>
      ...
      <div className="w-full flex flex-col border border-gray-200 rounded-xl my-2">
        ...
        <ul className="flex flex-col gap-3 mb-3 px-4 py-1">
            {
                Events.map((event) => (
                    <li key={event.id} className="w-full flex flex-col bg-[#27aa83]/5 hover:bg-[#27aa83]/10 rounded-xl p-2 cursor-pointer transition duration-200">
                        <h3>{event.title}</h3>
